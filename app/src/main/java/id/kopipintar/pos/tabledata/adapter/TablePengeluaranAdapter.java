...
import id.kopipintar.pos.tabledata.viewmodel.TablePengeluaranViewModel;

public class TablePengeluaranAdapter extends AbstractTableAdapter<ColumnHeaderModel, RowHeaderModel,
        CellModel> {

    private TablePengeluaranViewModel tablePengeluaranViewModel;

    public TablePengeluaranAdapter(Context context) {
        super(context);

        this.tablePengeluaranViewModel = new TablePengeluaranViewModel();
    }
    ...
    public AbstractViewHolder onCreateCellViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TablePengeluaranViewModel.BTN_TYPE){
            layout = LayoutInflater.from(mContext).inflate(R.layout.tableview_cell_button_layout,
                    parent, false);

            // Create a Cell ViewHolder
            return new BtnDelCellViewHolder(layout,2);
        }else{
    ...
    public int getCellItemViewType(int position) {
        return tablePengeluaranViewModel.getCellItemViewType(position);
    }

    public void setPengeluaranList(Pengeluaran pengeluarans) {
        // Generate the lists that are used to TableViewAdapter
        tablePengeluaranViewModel.generateListForTableView(pengeluarans);

        // Now we got what we need to show on TableView.
        setAllItems(tablePengeluaranViewModel.getColumHeaderModeList(), tablePengeluaranViewModel
                .getRowHeaderModelList(), tablePengeluaranViewModel.getCellModelList());
    }
